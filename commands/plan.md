...
> **Delegate + announce (keeps the main context lean).** From the **top-level command thread**, run substantive work as a **subagent via the Task tool** and **announce it first**: `→ delegating to <agent> because <reason>`. Here the `delivery-tracker` subagent does the decomposition; the main thread orchestrates + reports, so the subagent's CLI churn stays in its own context and the main window doesn't fill (context pressure compounds when auto-compaction hasn't fired). **No nested delegation** — a subagent (delivery-tracker, coder, …) does its work *inline*; it cannot spawn further subagents. **Token economy — prepend to every subagent prompt:** *"⚡ Respond in caveman/terse mode: drop articles/filler/pleasantries/hedging; fragments OK; keep ALL technical substance, exact `file:line`, code, and your VERDICT. Minimise output tokens."* (Verdict-bearing reviewers stay terse but never drop a finding/caveat.)
...
3. **Frame the design — architect co-leads (announce it).** For anything beyond a trivial change, delegate to the **pertinent designer** (the `architect`, or the §2b specialist — `azure-architect`/`dat-architect`/…): `→ delegating design framing to <architect> because the feature has load-bearing decisions`. It states the key decisions + writes/stubs the ADR (`docs/adr/NNNN-*.md`) that the US build against — so architecture leads development, not an afterthought. `delivery-tracker` owns the *board structure*; the architect owns the *design*. Skip only for a genuinely trivial/mechanical change (then say so).
