...
- **coder / specialist generator**: implement against the spec + ADR via the §2b build agent. **The generator skills ARE the specialized coders** — when the work matches one (`api-first`/`frontend-gen`/`iac-gen`/`auth-gen`/`observability-gen`/`data-model`/`db-migration`/`llm-gen`/`retriever`/`langgraph-workflow`/…), route to **that generator run as a subagent** (a coder subagent told to apply the skill, so its domain-correct output lands in `toolUseResult` and is attributable to the US — see §6); the generic `coder` subagent is the **fallback** when no generator fits. Either way it runs delegated (not in the main thread), returning files touched + what it ran.
