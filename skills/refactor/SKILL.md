...
## When to use
- "refactor this", "extract this into a function/module", "rename X everywhere", "split this god-class", "decompose this", "untangle this", "clean this up without changing behavior"
...
- Merge-readiness review of the resulting diff → `reviewer` (it FLAGS; refactor produces the change)
...
- The target code region/files + the transform requested (extract | inline | rename | move | decompose | dedup-to-helper)
...
3. **Name the transform + scope.** State exactly ONE transform class for this pass — `extract` | `inline` | `rename` | `move` | `decompose` | `dedup-to-helper` — and its concrete scope (files, symbols, call sites). One transform class per pass keeps the diff reviewable; mixing them defeats the proof.
