...
- [ ] the diff is structural ONLY — exactly one named transform (extract | inline | rename | move | decompose | dedup-to-helper), no public-signature/semantic/side-effect change, and no test file added, removed, or had an assertion altered: `git diff --name-only | grep -Eq '(test|spec)' && echo CHECK-TESTS-UNCHANGED || echo OK`
