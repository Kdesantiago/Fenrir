...
- [ ] flags reference Azure App Configuration with per-env labels (`dev`/`hml`/`prod`) and provider refresh enabled: `grep -rEq 'AppConfiguration|app(_|-)?config|select\(.*label|refresh' . && echo OK || echo MISSING`
...
## Functional
- The app resolves a kill-switch flag per `dev/hml/prod` label and a percentage/targeting filter parses; flipping the boolean in App Configuration disables the feature live within the refresh interval (no redeploy).
