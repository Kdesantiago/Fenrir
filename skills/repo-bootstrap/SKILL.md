...
3. **In-session guard + security layer (couche 0, agent-side)** — copy ALL of `hooks/*.py` → `.claude/hooks/`, copy `scripts/track_session.py` → `.claude/scripts/` (the delivery-tracking engine the hooks call), and MERGE `templates/.claude/settings.json` into the repo's `.claude/settings.json` (don't clobber existing hooks — append per event). Wires the pure-Python hooks: `delivery-guard` (PreToolUse — deny `--no-verify`/secret-exfil/zero-access paths, ask on force-push/gate-file edits), `prompt-guard` (UserPromptSubmit — injection scan), `content-scanner` (PostToolUse web — injection-in-content warning), `config-audit` (PostToolUse — audits `.claude/settings.json` changes), `session-context` (SessionStart — injects the active delivery contract + open gate-exceptions), plus the **delivery-tracking** quartet: `tracking-guard` (PreToolUse Bash — makes tracing a `git commit` obligatory: auto-creates a US, or denies in strict mode), `tracking-attribute` (PostToolUse Bash — on every `git commit`, flushes the cost since the last commit to the US that commit delivers, so each US carries its real per-US cost automatically), `tracking-collect` (SubagentStop — ledgers each subagent run for precise cost attribution), `tracking-finalize` (SessionEnd — backstop that attributes any remaining session cost to its US), `precompact-focus` (PreCompact — snapshots the active-US dev subject so a compaction stays on-topic; re-injected by `session-context` on `source=compact`), and `branch-plan-check` (PostToolUse Bash — advisory nudge to `/fenrir:plan` when you create a feature branch with no board plan). In-session twin of the pre-commit/CI gate; no bun/node/daemon. **Tracking needs the `dashboard/` companion app present**; without it the tracking hooks fail-open (no-op). Env knobs: `FENRIR_TRACK_ENFORCE=strict` (block untraced commits), `FENRIR_TRACK_DISABLE=1` (off).
...
5. **Branch-protection-as-code** — GitHub → `templates/branch-protection.tf`; Azure DevOps → `templates/azure-branch-policy.tf`. Fill repo/branch; `terraform apply` (this is the only thing that truly blocks a non-conforming merge). The required-check names MUST equal the CI job/stage names — verify the coupling. Also enable **auto-delete of merged branches** so they don't pile up: GitHub → `gh api -X PATCH repos/<owner>/<repo> -f delete_branch_on_merge=true` (Azure → repo policy "delete source branch" default); `/fenrir:ship` deletes the local branch after merge too (a squash-merge isn't seen by `git branch --merged`).
