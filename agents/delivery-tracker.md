...
- **Agile altitude — the rule that matters.**
  ...
  - **Feature = a business/dev capability** (a coherent deliverable a stakeholder would name) that GROUPS **several** atomic US. Not a session, not a grab-bag — and **not one US per feature**. Fragmenting work into many single-US features is an anti-pattern (harder to follow): bundle the related atomic US under one Feature. A 1-US feature is fine only when that US is genuinely a standalone capability; if an epic accumulates several single-US features, group them. `cli audit` surfaces this as `thin_features`.
